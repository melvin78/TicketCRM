...
{
    public class RoomCronJob:CronJobService
    {
        private readonly ILogger<AssignTicketCronJob> _logger;
        private readonly IServiceProvider _serviceProvider;

        public RoomCronJob(IScheduleConfig<AssignTicketCronJob> config,
        ...
        {
            _logger.LogInformation($"{DateTime.Now:hh:mm:ss} RoomCronJob Cron Job is working.");

            using (var scope = _serviceProvider.CreateScope())
            {
                var assignTicketService = scope.ServiceProvider.GetRequiredService<IAssignTicketService>();
                assignTicketService.CreateRoom(cancellationToken);
            }
        }
    }
}